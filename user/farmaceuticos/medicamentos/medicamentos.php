                ...
                <?php if (mysqli_num_rows($resultado) > 0): ?>
                    <?php while ($medicamento = mysqli_fetch_assoc($resultado)): ?>
                        <tr>
                            ...
                            <td>
                                <span class="badge 
                                    <?php echo $medicamento['estado'] === 'Disponible' ? 'bg-success' : 'bg-danger'; ?>">
                                    <?php echo htmlspecialchars($medicamento['estado']); ?>
                                </span>
                            </td>
                            <td>
                                <!-- Botón Editar -->
                                <a href="formumedicamento.php?id=<?php echo $medicamento['id']; ?>" class="btn btn-warning btn-sm" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <!-- Botón Eliminar -->
                                <a href="?eliminar=<?php echo $medicamento['id']; ?>" class="btn btn-danger btn-sm" title="Eliminar" onclick="return confirm('¿Está seguro de eliminar este medicamento?')">
